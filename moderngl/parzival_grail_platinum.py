#!/usr/bin/env python3
"""PLATINUM PACK -- The Grail Initiation"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_parzival_grail")
...
def v_the_fool_who_does_no(im, u, t, p):
    ...
    seal(im,"The Fool Who Does Not Know","fool to king")

def v_the_mother_who_hides(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Mother Who Hides Knighthood","fool to king")

def v_the_red_knight_defea(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Red Knight Defeated","fool to king")

def v_the_court_of_arthur(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Court of Arthur","fool to king")

def v_condwiramurs(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Condwiramurs","fool to king")

def v_the_wounded_fisher_k(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Wounded Fisher King","fool to king")

def v_the_grail_castle(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Grail Castle","fool to king")

def v_the_unasked_question(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Unasked Question","fool to king")

def v_the_waste_land(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Waste Land","fool to king")

def v_redemption(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Redemption","fool to king")

def v_the_temple_of_the_gr(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Temple of the Grail","fool to king")

def v_becoming_the_grail(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Becoming the Grail","fool to king")

VISUALS = {
    "the_fool_who_does_no": v_the_fool_who_does_no,
    "the_mother_who_hides": v_the_mother_who_hides,
    "the_red_knight_defea": v_the_red_knight_defea,
    "the_court_of_arthur": v_the_court_of_arthur,
    "condwiramurs": v_condwiramurs,
    "the_wounded_fisher_k": v_the_wounded_fisher_k,
    "the_grail_castle": v_the_grail_castle,
    "the_unasked_question": v_the_unasked_question,
    "the_waste_land": v_the_waste_land,
    "redemption": v_redemption,
    "the_temple_of_the_gr": v_the_temple_of_the_gr,
    "becoming_the_grail": v_becoming_the_grail,
}
...
SCENES = [
    Scene("The Fool Who Does Not Know", "fool to king", 7.0, "the_fool_who_does_no", {}),
    Scene("The Mother Who Hides Knighthood", "fool to king", 7.0, "the_mother_who_hides", {}),
    Scene("The Red Knight Defeated", "fool to king", 7.0, "the_red_knight_defea", {}),
    Scene("The Court of Arthur", "fool to king", 7.0, "the_court_of_arthur", {}),
    Scene("Condwiramurs", "fool to king", 7.0, "condwiramurs", {}),
    Scene("The Wounded Fisher King", "fool to king", 7.0, "the_wounded_fisher_k", {}),
    Scene("The Grail Castle", "fool to king", 7.0, "the_grail_castle", {}),
    Scene("The Unasked Question", "fool to king", 7.0, "the_unasked_question", {}),
    Scene("The Waste Land", "fool to king", 7.0, "the_waste_land", {}),
    Scene("Redemption", "fool to king", 7.0, "redemption", {}),
    Scene("The Temple of the Grail", "fool to king", 7.0, "the_temple_of_the_gr", {}),
    Scene("Becoming the Grail", "fool to king", 7.0, "becoming_the_grail", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"parzival_grail.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Grail Initiation","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Grail Initiation",
        "palette_roles":{"gold":"grail","cyan":"quest","violet":"temple"},"scenes":recs},
