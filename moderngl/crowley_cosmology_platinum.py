#!/usr/bin/env python3
"""PLATINUM PACK -- The Book of the Law"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_crowley_cosmology")
...
def v_nuit_infinite_space(im, u, t, p):
    ...
    seal(im,"Nuit Infinite Space","Nuit Hadit Ra-Hoor-Khuit")

def v_hadit_the_point(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Hadit The Point","Nuit Hadit Ra-Hoor-Khuit")

def v_ra_hoor_khuit_the_un(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Ra-Hoor-Khuit The Union","Nuit Hadit Ra-Hoor-Khuit")

def v_do_what_thou_wilt(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Do What Thou Wilt","Nuit Hadit Ra-Hoor-Khuit")

def v_love_under_will(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Love Under Will","Nuit Hadit Ra-Hoor-Khuit")

def v_the_aeon_of_horus(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Aeon of Horus","Nuit Hadit Ra-Hoor-Khuit")

def v_the_eight_trigrams(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Eight Trigrams","Nuit Hadit Ra-Hoor-Khuit")

def v_liber_al(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Liber AL","Nuit Hadit Ra-Hoor-Khuit")

def v_the_solar_phallus(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Solar Phallus","Nuit Hadit Ra-Hoor-Khuit")

def v_the_serpent_power(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Serpent Power","Nuit Hadit Ra-Hoor-Khuit")

VISUALS = {
    "nuit_infinite_space": v_nuit_infinite_space,
    "hadit_the_point": v_hadit_the_point,
    "ra_hoor_khuit_the_un": v_ra_hoor_khuit_the_un,
    "do_what_thou_wilt": v_do_what_thou_wilt,
    "love_under_will": v_love_under_will,
    "the_aeon_of_horus": v_the_aeon_of_horus,
    "the_eight_trigrams": v_the_eight_trigrams,
    "liber_al": v_liber_al,
    "the_solar_phallus": v_the_solar_phallus,
    "the_serpent_power": v_the_serpent_power,
}
...
SCENES = [
    Scene("Nuit Infinite Space", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "nuit_infinite_space", {}),
    Scene("Hadit The Point", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "hadit_the_point", {}),
    Scene("Ra-Hoor-Khuit The Union", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "ra_hoor_khuit_the_un", {}),
    Scene("Do What Thou Wilt", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "do_what_thou_wilt", {}),
    Scene("Love Under Will", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "love_under_will", {}),
    Scene("The Aeon of Horus", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "the_aeon_of_horus", {}),
    Scene("The Eight Trigrams", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "the_eight_trigrams", {}),
    Scene("Liber AL", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "liber_al", {}),
    Scene("The Solar Phallus", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "the_solar_phallus", {}),
    Scene("The Serpent Power", "Nuit Hadit Ra-Hoor-Khuit", 7.0, "the_serpent_power", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"crowley_cosmology.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Book of the Law","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Book of the Law",
        "palette_roles":{"gold":"hadit","violet":"nuit","crimson":"rahoor"},"scenes":recs},
