#!/usr/bin/env python3
"""BREATH IS THE CLOCK THE BRAIN CANNOT IGNORE — Platinum visual essay"""
...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_breath_clock"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v_breath_wave(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    pts=[]
    for i in range(150):
        u2=i/149; x=lerp(150,1130,u2); y=cy+math.sin(u2*2*math.pi+t*1.5)*40*pr
        pts.append((x,y))
    gl(im,pp(pts,pr),TEAL,4,14,210)
    gc(im,cx,cy-30,20,GL,int(80*pr),14)
    seal(im,"BREATH IS THE CLOCK THE BRAIN CANNOT IGNORE","respiration entrains neural rhythm — the forebrain pulses with the breath")

def v_cycle(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-90,cy-50,cx+90,cy+50),outline=(*INK,int(160*pr)),width=3)
    for i,(lab,col) in enumerate([("INHALE",TEAL),("PAUSE",GOLD),("EXHALE",CRIMSON),("STILL",VIOLET)]):
        a=-math.pi/2+i*math.pi/2; x=cx+math.cos(a)*130; y=cy+math.sin(a)*90
        q=clamp(pr*1.3-i*.07)
        if q<=0: continue
        d.ellipse((x-12,y-12,x+12,y+12),outline=(*col,int(160*q)),fill=(*mix(IVORY,col,.1),int(120*q)),width=2)
        ct(d,(x,y),lab,lf(FSNB,int(h*.015)),col)
    seal(im,"INHALE — PAUSE — EXHALE — STILL","the four phases of breath structure the rhythm of awareness")

def v_forebrain(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-60,cy-40,cx+60,cy+40),outline=(*TEAL,int(170*pr)),width=3)
    gc(im,cx,cy,20,GL,int(80*pr),14)
    for i in range(6):
        a=i*2*math.pi/6+t*.05; r=lerp(10,80,pr)*math.sin(t*2+i)
        x=cx+math.cos(a)*r; y=cy+math.sin(a)*r*.6
        col=mix(TEAL,GOLD,i/5); d.ellipse((x-4,y-4,x+4,y+4),fill=(*col,int(130*pr)))
    ct(d,(cx,cy),"FOREBRAIN",lf(FSNB,int(h*.018)),TEAL)
    seal(im,"THE FOREBRAIN PULSES WITH THE BREATH","respiratory phase shapes memory, attention, and affect")

def v_21600(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    ct(d,(cx,cy-30),"21,600",lf(FSB,int(h*.050)),GOLD)
    ct(d,(cx,cy+20),"BREATHS PER DAY",lf(FSNB,int(h*.022)),SOFT_INK)
    for i in range(12):
        a=-math.pi/2+i*2*math.pi/12; r=160*pr
        x=cx+math.cos(a)*r; y=cy+math.sin(a)*r*.6
        d.ellipse((x-3,y-3,x+3,y+3),fill=(*GOLD,int(130*pr)))
    seal(im,"THE RITUAL NUMBER 21,600","breath phases become lunar goddesses — the body counts without counting")
...
def v_closing(im,u,t,p):
    ...
    seal(im,"BREATH IS THE CLOCK THE BRAIN CANNOT IGNORE","the body counts without counting — each breath is a beat in the pulse of consciousness")

VISUALS={"breath_wave":v_breath_wave,"cycle":v_cycle,"forebrain":v_forebrain,"21600":v_21600,"closing":v_closing}
SCENES=[
    Scene("Breath Wave","Breath is the clock the brain cannot ignore — respiration entrains neural rhythm.",5.5,"breath_wave",{}),
    Scene("Entrainment","Each breath, the forebrain pulses — memory indexed by inhalation and exhalation.",5.5,"breath_wave",{}),
    Scene("Four Phases","Inhale — pause — exhale — still — four phases structure the rhythm of awareness.",5.5,"cycle",{}),
    Scene("Forebrain","The forebrain pulses with the breath — respiratory phase shapes memory, attention, affect.",5.5,"forebrain",{}),
    Scene("21600","21,600 breaths per day — the ritual number, not a universal physiological rate.",5.5,"21600",{}),
    Scene("Goddesses","Breath phases become lunar goddesses in the tantric imagination.",5.5,"21600",{}),
    Scene("Counting","The body counts without counting — each breath is a beat in the pulse of consciousness.",5.5,"breath_wave",{}),
    Scene("Closing","Breath is the clock the brain cannot ignore — the body counts without counting.",6.0,"closing",{}),
    Scene("Final","Each breath is a beat in the pulse of consciousness — you have been keeping time all along.",6.5,"closing",{}),
]
...
def export_tl():
    ...
    p=O/"narration_timeline.json"; p.write_text(json.dumps({"title":"breath is the clock the brain cannot ignore","runtime_seconds":round(cur,3),"scene_count":len(SCENES),"scenes":pl},indent=2,ensure_ascii=False),encoding="utf-8"); return p
def cs(w,h):
