#!/usr/bin/env python3
"""A BODY MAY REMEMBER AN IDEA — Platinum visual essay"""
...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_body_remembers_idea"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
VIOLET=(130,104,160); LAPIS=(56,76,124); DARK=(24,27,32); GREEN=(68,139,99)
...
def v_idea_shape(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-90,cy-60,cx+90,cy+60),outline=(*INK,int(170*pr)),width=3)
    gc(im,cx,cy,22,GL,int(80*pr),14)
    d.ellipse((cx-40,cy-25,cx+40,cy+25),fill=(*TEAL,int(40*pr)))
    ct(d,(cx,cy),"IDEA",lf(FSB,int(h*.030)),TEAL)
    seal(im,"A BODY MAY REMEMBER AN IDEA","the form of a thought is not only mental — it is also molecular, postural, lived")

def v_morphic(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    for i in range(5):
        y=150+i*75; q=clamp(pr*1.3-i*.06)
        if q<=0: continue
        col=mix(GOLD,TEAL,i/4); d.ellipse((cx-100+40*pr,y-18,cx+100-40*pr,y+18),outline=(*col,int(160*q)),width=2)
        ct(d,(cx,y),["PLATONIC FORM","BIOLOGICAL FORM","BIOELECTRIC FIELD","MATURE ANATOMY","REGENERATIVE MEMORY"][i],lf(FSN,int(h*.014)),col)
    seal(im,"FROM FORM TO BIOLOGY TO MEMORY","the Platonic form meets the morphospace — the idea becomes a body")

def v_limb(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-80,cy-60,cx+80,cy+60),outline=(*INK,int(160*pr)),width=3)
    d.ellipse((cx-90,cy-10,cx+90,cy+50),outline=(*SILVER,int(130*pr)),width=2)
    ct(d,(cx,cy),"LIMB AS NEGATIVE SPACE",lf(FSNB,int(h*.020)),SOFT_INK)
    gc(im,cx,cy+30,15,GL,int(80*pr),12)
    seal(im,"THE MISSING LIMB AS NEGATIVE SPACE","the form is present in its absence — the body knows what is not there")

def v_closing(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    for i,(r,c) in enumerate([(200,GOLD),(150,TEAL),(100,VIOLET),(50,GREEN)]): rr=r*pr; d.ellipse((cx-rr,cy-60-rr*.5,cx+rr,cy-60+rr*.5),outline=(*c,int(120-20*(r//50))),width=2)
    ...
    seal(im,"A BODY MAY REMEMBER AN IDEA","not only the mind holds the form — the body is the idea's dwelling")

VISUALS={"idea_shape":v_idea_shape,"morphic":v_morphic,"limb":v_limb,"closing":v_closing}
SCENES=[
    Scene("Idea","A body may remember an idea — the form of a thought is molecular, postural, lived.",5.5,"idea_shape",{}),
    Scene("Form","Not only mental — the idea has a shape the body knows.",5.5,"idea_shape",{}),
    Scene("Platonic","From Platonic form to biological morphospace — the idea becomes a body.",5.5,"morphic",{}),
    Scene("Limb","The missing limb as negative space — the body knows what is not there.",5.5,"limb",{}),
    Scene("Absence","The form is present in its absence — the body remembers the shape it no longer wears.",5.5,"limb",{}),
    Scene("Closing","A body may remember an idea — the body is the idea's dwelling.",6.0,"closing",{}),
    Scene("Final","Not only the mind holds the form — the body is the idea's dwelling and guardian.",6.5,"closing",{}),
]
...
def export_tl():
    ...
    p=O/"narration_timeline.json"; p.write_text(json.dumps({"title":"a body may remember an idea","runtime_seconds":round(cur,3),"scene_count":len(SCENES),"scenes":pl},indent=2,ensure_ascii=False),encoding="utf-8"); return p
def cs(w,h):
