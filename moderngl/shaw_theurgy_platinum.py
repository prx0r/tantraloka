#!/usr/bin/env python3
"""PLATINUM PACK -- Theurgy as Divine Embodiment"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_shaw_theurgy")
...
def v_iamblichus_against_p(im, u, t, p):
    ...
    seal(im,"Iamblichus Against Porphyry","Iamblichus ritual")

def v_the_two_souls(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Two Souls","Iamblichus ritual")

def v_the_symbol(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Symbol","Iamblichus ritual")

def v_theurgy_as_embodimen(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Theurgy as Embodiment","Iamblichus ritual")

def v_why_matter_saves(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Why Matter Saves","Iamblichus ritual")

def v_the_ladder(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Ladder","Iamblichus ritual")

def v_containing_ecstasy(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Containing Ecstasy","Iamblichus ritual")

def v_participation(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Participation","Iamblichus ritual")

def v_embodying_the_stars(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Embodying the Stars","Iamblichus ritual")

def v_tantric_parallel(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Tantric Parallel","Iamblichus ritual")

VISUALS = {
    "iamblichus_against_p": v_iamblichus_against_p,
    "the_two_souls": v_the_two_souls,
    "the_symbol": v_the_symbol,
    "theurgy_as_embodimen": v_theurgy_as_embodimen,
    "why_matter_saves": v_why_matter_saves,
    "the_ladder": v_the_ladder,
    "containing_ecstasy": v_containing_ecstasy,
    "participation": v_participation,
    "embodying_the_stars": v_embodying_the_stars,
    "tantric_parallel": v_tantric_parallel,
}
...
SCENES = [
    Scene("Iamblichus Against Porphyry", "Iamblichus ritual", 7.0, "iamblichus_against_p", {}),
    Scene("The Two Souls", "Iamblichus ritual", 7.0, "the_two_souls", {}),
    Scene("The Symbol", "Iamblichus ritual", 7.0, "the_symbol", {}),
    Scene("Theurgy as Embodiment", "Iamblichus ritual", 7.0, "theurgy_as_embodimen", {}),
    Scene("Why Matter Saves", "Iamblichus ritual", 7.0, "why_matter_saves", {}),
    Scene("The Ladder", "Iamblichus ritual", 7.0, "the_ladder", {}),
    Scene("Containing Ecstasy", "Iamblichus ritual", 7.0, "containing_ecstasy", {}),
    Scene("Participation", "Iamblichus ritual", 7.0, "participation", {}),
    Scene("Embodying the Stars", "Iamblichus ritual", 7.0, "embodying_the_stars", {}),
    Scene("Tantric Parallel", "Iamblichus ritual", 7.0, "tantric_parallel", {}),
]
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"Theurgy as Divine Embodiment","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBTheurgy as Divine Embodiment",
        "palette_roles":{"gold":"divine","cyan":"symbol","violet":"priest"},"scenes":recs},
