#!/usr/bin/env python3
"""PLATINUM PACK -- The Three Upayas"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_three_upayas")
...
def v_anavopaya_the_indivi(im, u, t, p):
    ...
    seal(im,"Anavopaya The Individual Means","paths to liberation")

def v_the_path_of_effort(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Path of Effort","paths to liberation")

def v_saktopaya_the_means_(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Saktopaya The Means of Energy","paths to liberation")

def v_the_path_of_power(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Path of Power","paths to liberation")

def v_sambhavopaya_the_div(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Sambhavopaya The Divine Means","paths to liberation")

def v_sudden_recognition(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Sudden Recognition","paths to liberation")

def v_anupaya_no_means(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Anupaya No Means","paths to liberation")

def v_the_pathless_path(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Pathless Path","paths to liberation")

def v_grace_and_the_teache(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Grace and the Teacher","paths to liberation")

def v_the_nondual_heart(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Nondual Heart","paths to liberation")

VISUALS = {
    "anavopaya_the_indivi": v_anavopaya_the_indivi,
    "the_path_of_effort": v_the_path_of_effort,
    "saktopaya_the_means_": v_saktopaya_the_means_,
    "the_path_of_power": v_the_path_of_power,
    "sambhavopaya_the_div": v_sambhavopaya_the_div,
    "sudden_recognition": v_sudden_recognition,
    "anupaya_no_means": v_anupaya_no_means,
    "the_pathless_path": v_the_pathless_path,
    "grace_and_the_teache": v_grace_and_the_teache,
    "the_nondual_heart": v_the_nondual_heart,
}
...
SCENES = [
    Scene("Anavopaya The Individual Means", "paths to liberation", 7.0, "anavopaya_the_indivi", {}),
    Scene("The Path of Effort", "paths to liberation", 7.0, "the_path_of_effort", {}),
    Scene("Saktopaya The Means of Energy", "paths to liberation", 7.0, "saktopaya_the_means_", {}),
    Scene("The Path of Power", "paths to liberation", 7.0, "the_path_of_power", {}),
    Scene("Sambhavopaya The Divine Means", "paths to liberation", 7.0, "sambhavopaya_the_div", {}),
    Scene("Sudden Recognition", "paths to liberation", 7.0, "sudden_recognition", {}),
    Scene("Anupaya No Means", "paths to liberation", 7.0, "anupaya_no_means", {}),
    Scene("The Pathless Path", "paths to liberation", 7.0, "the_pathless_path", {}),
    Scene("Grace and the Teacher", "paths to liberation", 7.0, "grace_and_the_teache", {}),
    Scene("The Nondual Heart", "paths to liberation", 7.0, "the_nondual_heart", {}),
]
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Three Upayas","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Three Upayas",
        "palette_roles":{"gold":"sambhava","cyan":"sakta","violet":"anava"},"scenes":recs},
