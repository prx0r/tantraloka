#!/usr/bin/env python3
"""PLATINUM PACK -- The Mundus Imaginalis"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_corbin_imaginal")
...
def v_the_mundus_imaginali(im, u, t, p):
    ...
    seal(im,"The Mundus Imaginalis","the world between")

def v_hurqalya(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Hurqalya","the world between")

def v_perfect_nature(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Perfect Nature","the world between")

def v_the_1x1_formula(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The 1x1 Formula","the world between")

def v_call_me_by_my_name(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Call Me By My Name","the world between")

def v_the_witness_in_heave(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Witness in Heaven","the world between")

def v_the_suprasensory_gui(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Suprasensory Guide","the world between")

def v_the_dhikr_destroys_d(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Dhikr Destroys Darkness","the world between")

def v_the_cosmic_north(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Cosmic North","the world between")

def v_the_daena_at_the_bri(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Daena at the Bridge","the world between")

VISUALS = {
    "the_mundus_imaginali": v_the_mundus_imaginali,
    "hurqalya": v_hurqalya,
    "perfect_nature": v_perfect_nature,
    "the_1x1_formula": v_the_1x1_formula,
    "call_me_by_my_name": v_call_me_by_my_name,
    "the_witness_in_heave": v_the_witness_in_heave,
    "the_suprasensory_gui": v_the_suprasensory_gui,
    "the_dhikr_destroys_d": v_the_dhikr_destroys_d,
    "the_cosmic_north": v_the_cosmic_north,
    "the_daena_at_the_bri": v_the_daena_at_the_bri,
}
...
SCENES = [
    Scene("The Mundus Imaginalis", "the world between", 7.0, "the_mundus_imaginali", {}),
    Scene("Hurqalya", "the world between", 7.0, "hurqalya", {}),
    Scene("Perfect Nature", "the world between", 7.0, "perfect_nature", {}),
    Scene("The 1x1 Formula", "the world between", 7.0, "the_1x1_formula", {}),
    Scene("Call Me By My Name", "the world between", 7.0, "call_me_by_my_name", {}),
    Scene("The Witness in Heaven", "the world between", 7.0, "the_witness_in_heave", {}),
    Scene("The Suprasensory Guide", "the world between", 7.0, "the_suprasensory_gui", {}),
    Scene("The Dhikr Destroys Darkness", "the world between", 7.0, "the_dhikr_destroys_d", {}),
    Scene("The Cosmic North", "the world between", 7.0, "the_cosmic_north", {}),
    Scene("The Daena at the Bridge", "the world between", 7.0, "the_daena_at_the_bri", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"corbin_imaginal.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Mundus Imaginalis","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Mundus Imaginalis",
        "palette_roles":{"gold":"imaginal","violet":"guide"},"scenes":recs},
