#!/usr/bin/env python3
"""PLATINUM PACK -- Poimandres and the Hermetic Revelation"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_corpus_hermeticum")
...
def v_the_vision_of_poiman(im, u, t, p):
    ...
    seal(im,"The Vision of Poimandres","the seven spheres of ascent")

def v_the_seven_spheres_go(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Seven Spheres Go Wrong","the seven spheres of ascent")

def v_man_breaks_through_t(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Man Breaks Through the Vault","the seven spheres of ascent")

def v_nature_and_the_first(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Nature and the First Man","the seven spheres of ascent")

def v_the_seven_governors(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Seven Governors","the seven spheres of ascent")

def v_the_mystery_of_birth(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Mystery of Birth","the seven spheres of ascent")

def v_the_soul_ascent(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Soul Ascent","the seven spheres of ascent")

def v_gnosis_is_the_only_s(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Gnosis is the Only Salvation","the seven spheres of ascent")

def v_god_is_the_light(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"God is the Light","the seven spheres of ascent")
...
def v_as_above_so_below(im, u, t, p):
    ...
    for j in range(int(10*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"As Above, So Below","the seven spheres of ascent")

VISUALS = {
    "the_vision_of_poiman": v_the_vision_of_poiman,
    "the_seven_spheres_go": v_the_seven_spheres_go,
    "man_breaks_through_t": v_man_breaks_through_t,
    "nature_and_the_first": v_nature_and_the_first,
    "the_seven_governors": v_the_seven_governors,
    "the_mystery_of_birth": v_the_mystery_of_birth,
    "the_soul_ascent": v_the_soul_ascent,
    "gnosis_is_the_only_s": v_gnosis_is_the_only_s,
    "god_is_the_light": v_god_is_the_light,
...
SCENES = [
    Scene("The Vision of Poimandres", "the seven spheres of ascent", 7.0, "the_vision_of_poiman", {}),
    Scene("The Seven Spheres Go Wrong", "the seven spheres of ascent", 7.0, "the_seven_spheres_go", {}),
    Scene("Man Breaks Through the Vault", "the seven spheres of ascent", 7.0, "man_breaks_through_t", {}),
    Scene("Nature and the First Man", "the seven spheres of ascent", 7.0, "nature_and_the_first", {}),
    Scene("The Seven Governors", "the seven spheres of ascent", 7.0, "the_seven_governors", {}),
    Scene("The Mystery of Birth", "the seven spheres of ascent", 7.0, "the_mystery_of_birth", {}),
    Scene("The Soul Ascent", "the seven spheres of ascent", 7.0, "the_soul_ascent", {}),
    Scene("Gnosis is the Only Salvation", "the seven spheres of ascent", 7.0, "gnosis_is_the_only_s", {}),
    Scene("God is the Light", "the seven spheres of ascent", 7.0, "god_is_the_light", {}),
    Scene("As Above, So Below", "the seven spheres of ascent", 7.0, "as_above_so_below", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"corpus_hermeticum.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"Poimandres and the Hermetic Revelation","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBPoimandres and the Hermetic Revelation",
        "palette_roles":{"gold":"nous","cyan":"the spheres"},"scenes":recs},
