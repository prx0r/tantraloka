#!/usr/bin/env python3
"""PLATINUM PACK -- The 50 Phonemes of Consciousness"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_sound_metaphysics")
...
def v_the_four_levels_of_s(im, u, t, p):
    ...
    seal(im,"The Four Levels of Speech","speech as fabric of reality")

def v_vaikhari_audible(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Vaikhari Audible","speech as fabric of reality")

def v_madhyama_mental(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Madhyama Mental","speech as fabric of reality")

def v_pasyanti_visionary(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Pasyanti Visionary","speech as fabric of reality")

def v_para_transcendent(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Para Transcendent","speech as fabric of reality")

def v_the_50_phonemes(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The 50 Phonemes","speech as fabric of reality")

def v_the_matrka_cakra(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Matrka Cakra","speech as fabric of reality")

def v_the_malini_cakra(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Malini Cakra","speech as fabric of reality")

def v_the_a_ha_cycle(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The A-Ha Cycle","speech as fabric of reality")

def v_the_dvadasanta(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Dvadasanta","speech as fabric of reality")

def v_kundalini_as_sound(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Kundalini as Sound","speech as fabric of reality")

def v_mantra_as_living_pow(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Mantra as Living Power","speech as fabric of reality")

VISUALS = {
    "the_four_levels_of_s": v_the_four_levels_of_s,
    "vaikhari_audible": v_vaikhari_audible,
    "madhyama_mental": v_madhyama_mental,
    "pasyanti_visionary": v_pasyanti_visionary,
    "para_transcendent": v_para_transcendent,
    "the_50_phonemes": v_the_50_phonemes,
    "the_matrka_cakra": v_the_matrka_cakra,
    "the_malini_cakra": v_the_malini_cakra,
    "the_a_ha_cycle": v_the_a_ha_cycle,
    "the_dvadasanta": v_the_dvadasanta,
    "kundalini_as_sound": v_kundalini_as_sound,
    "mantra_as_living_pow": v_mantra_as_living_pow,
}
...
SCENES = [
    Scene("The Four Levels of Speech", "speech as fabric of reality", 7.0, "the_four_levels_of_s", {}),
    Scene("Vaikhari Audible", "speech as fabric of reality", 7.0, "vaikhari_audible", {}),
    Scene("Madhyama Mental", "speech as fabric of reality", 7.0, "madhyama_mental", {}),
    Scene("Pasyanti Visionary", "speech as fabric of reality", 7.0, "pasyanti_visionary", {}),
    Scene("Para Transcendent", "speech as fabric of reality", 7.0, "para_transcendent", {}),
    Scene("The 50 Phonemes", "speech as fabric of reality", 7.0, "the_50_phonemes", {}),
    Scene("The Matrka Cakra", "speech as fabric of reality", 7.0, "the_matrka_cakra", {}),
    Scene("The Malini Cakra", "speech as fabric of reality", 7.0, "the_malini_cakra", {}),
    Scene("The A-Ha Cycle", "speech as fabric of reality", 7.0, "the_a_ha_cycle", {}),
    Scene("The Dvadasanta", "speech as fabric of reality", 7.0, "the_dvadasanta", {}),
    Scene("Kundalini as Sound", "speech as fabric of reality", 7.0, "kundalini_as_sound", {}),
    Scene("Mantra as Living Power", "speech as fabric of reality", 7.0, "mantra_as_living_pow", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"sound_metaphysics.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The 50 Phonemes of Consciousness","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe 50 Phonemes of Consciousness",
        "palette_roles":{"gold":"para","cyan":"pasyanti","violet":"madhyama"},"scenes":recs},
