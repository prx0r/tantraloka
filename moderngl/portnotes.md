...
Instead of live audio analysis (Synesthesia model), we precompute per-frame
audio features with librosa: volume envelope, onset strength, spectral centroid.
These are baked to a JSON/npy array at render time, then passed as uniforms
(`u_audioVolume`, `u_audioBeat`, `u_audioSpectral`) per frame — the same
pattern as `u` and `t`.

Our narration WAVs are already generated by edge-tts. The audio analysis lives
in `moderngl/renderer/audio_analysis.py` and outputs per-frame feature arrays
matching the render's fps.
...
- Audio-reactive uniform wiring in shaders
