#!/usr/bin/env python3
"""PLATINUM PACK -- The Starry Daimon"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_ficino_daimon")
...
def v_the_renaissance_of_t(im, u, t, p):
    ...
    seal(im,"The Renaissance of the Daimon","Renaissance star magic")

def v_ficino_plato(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Ficino Plato","Renaissance star magic")

def v_the_tutelary_spirit(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Tutelary Spirit","Renaissance star magic")

def v_the_stars(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Stars","Renaissance star magic")

def v_orphic_hymns(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Orphic Hymns","Renaissance star magic")

def v_solar_hymn(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Solar Hymn","Renaissance star magic")

def v_saturnine_retreat(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Saturnine Retreat","Renaissance star magic")

def v_spiritus(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Spiritus","Renaissance star magic")

def v_amor_daimonic(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Amor Daimonic","Renaissance star magic")

def v_the_practice(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Practice","Renaissance star magic")

VISUALS = {
    "the_renaissance_of_t": v_the_renaissance_of_t,
    "ficino_plato": v_ficino_plato,
    "the_tutelary_spirit": v_the_tutelary_spirit,
    "the_stars": v_the_stars,
    "orphic_hymns": v_orphic_hymns,
    "solar_hymn": v_solar_hymn,
    "saturnine_retreat": v_saturnine_retreat,
    "spiritus": v_spiritus,
    "amor_daimonic": v_amor_daimonic,
    "the_practice": v_the_practice,
}
...
SCENES = [
    Scene("The Renaissance of the Daimon", "Renaissance star magic", 7.0, "the_renaissance_of_t", {}),
    Scene("Ficino Plato", "Renaissance star magic", 7.0, "ficino_plato", {}),
    Scene("The Tutelary Spirit", "Renaissance star magic", 7.0, "the_tutelary_spirit", {}),
    Scene("The Stars", "Renaissance star magic", 7.0, "the_stars", {}),
    Scene("Orphic Hymns", "Renaissance star magic", 7.0, "orphic_hymns", {}),
    Scene("Solar Hymn", "Renaissance star magic", 7.0, "solar_hymn", {}),
    Scene("Saturnine Retreat", "Renaissance star magic", 7.0, "saturnine_retreat", {}),
    Scene("Spiritus", "Renaissance star magic", 7.0, "spiritus", {}),
    Scene("Amor Daimonic", "Renaissance star magic", 7.0, "amor_daimonic", {}),
    Scene("The Practice", "Renaissance star magic", 7.0, "the_practice", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"ficino_daimon.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Starry Daimon","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Starry Daimon",
        "palette_roles":{"gold":"daimon","violet":"spheres","cyan":"music"},"scenes":recs},
