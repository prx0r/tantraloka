#!/usr/bin/env python3
"""PLATINUM PACK -- The Doctrine of Vibration"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_vibration")
...
def v_spanda_universal_pul(im, u, t, p):
    ...
    seal(im,"Spanda Universal Pulse","spanda as pulse")

def v_prakasha_light_of_co(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Prakasha Light of Consciousness","spanda as pulse")

def v_vimarsha_reflective_(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Vimarsha Reflective Awareness","spanda as pulse")

def v_siva_sakti(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Siva-Sakti","spanda as pulse")

def v_iccha_will(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Iccha Will","spanda as pulse")

def v_jnana_knowledge(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Jnana Knowledge","spanda as pulse")

def v_kriya_action(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Kriya Action","spanda as pulse")

def v_the_wheel_of_energie(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Wheel of Energies","spanda as pulse")

def v_pratyabhijna_recogni(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Pratyabhijna Recognition","spanda as pulse")

def v_divine_body(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Divine Body","spanda as pulse")
...
def v_freedom(im, u, t, p):
    ...
    for j in range(int(10*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Freedom","spanda as pulse")

VISUALS = {
    "spanda_universal_pul": v_spanda_universal_pul,
    "prakasha_light_of_co": v_prakasha_light_of_co,
    "vimarsha_reflective_": v_vimarsha_reflective_,
    "siva_sakti": v_siva_sakti,
    "iccha_will": v_iccha_will,
    "jnana_knowledge": v_jnana_knowledge,
    "kriya_action": v_kriya_action,
    "the_wheel_of_energie": v_the_wheel_of_energie,
    "pratyabhijna_recogni": v_pratyabhijna_recogni,
    "divine_body": v_divine_body,
    "freedom": v_freedom,
}
...
SCENES = [
    Scene("Spanda Universal Pulse", "spanda as pulse", 7.0, "spanda_universal_pul", {}),
    Scene("Prakasha Light of Consciousness", "spanda as pulse", 7.0, "prakasha_light_of_co", {}),
    Scene("Vimarsha Reflective Awareness", "spanda as pulse", 7.0, "vimarsha_reflective_", {}),
    Scene("Siva-Sakti", "spanda as pulse", 7.0, "siva_sakti", {}),
    Scene("Iccha Will", "spanda as pulse", 7.0, "iccha_will", {}),
    Scene("Jnana Knowledge", "spanda as pulse", 7.0, "jnana_knowledge", {}),
    Scene("Kriya Action", "spanda as pulse", 7.0, "kriya_action", {}),
    Scene("The Wheel of Energies", "spanda as pulse", 7.0, "the_wheel_of_energie", {}),
    Scene("Pratyabhijna Recognition", "spanda as pulse", 7.0, "pratyabhijna_recogni", {}),
    Scene("Divine Body", "spanda as pulse", 7.0, "divine_body", {}),
    Scene("Freedom", "spanda as pulse", 7.0, "freedom", {}),
]
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Doctrine of Vibration","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Doctrine of Vibration",
        "palette_roles":{"gold":"spanda","violet":"prakasha","cyan":"vimarsha"},"scenes":recs},
