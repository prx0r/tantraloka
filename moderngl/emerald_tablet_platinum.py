#!/usr/bin/env python3
"""PLATINUM PACK -- The Emerald Tablet"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_emerald_tablet")
...
def v_the_whole_truth(im, u, t, p):
    ...
    seal(im,"The Whole Truth","as above, so below")

def v_as_above_so_below(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"As Above So Below","as above, so below")

def v_the_father_is_the_su(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Father is the Sun","as above, so below")

def v_the_mother_is_the_mo(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Mother is the Moon","as above, so below")

def v_the_wind_carried_it(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Wind Carried It","as above, so below")

def v_the_earth_is_its_nur(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Earth is Its Nurse","as above, so below")

def v_separated_from_the_g(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Separated from the Gross","as above, so below")

def v_it_ascends_and_desce(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"It Ascends and Descends","as above, so below")

def v_the_strength_of_all_(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Strength of All Strength","as above, so below")

def v_the_world_was_create(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The World Was Created","as above, so below")

def v_transformed_into_one(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Transformed into One","as above, so below")

def v_leaving_the_gross_be(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Leaving the Gross Behind","as above, so below")

VISUALS = {
    "the_whole_truth": v_the_whole_truth,
    "as_above_so_below": v_as_above_so_below,
    "the_father_is_the_su": v_the_father_is_the_su,
    "the_mother_is_the_mo": v_the_mother_is_the_mo,
    "the_wind_carried_it": v_the_wind_carried_it,
    "the_earth_is_its_nur": v_the_earth_is_its_nur,
    "separated_from_the_g": v_separated_from_the_g,
    "it_ascends_and_desce": v_it_ascends_and_desce,
    "the_strength_of_all_": v_the_strength_of_all_,
    "the_world_was_create": v_the_world_was_create,
    "transformed_into_one": v_transformed_into_one,
    "leaving_the_gross_be": v_leaving_the_gross_be,
}
...
SCENES = [
    Scene("The Whole Truth", "as above, so below", 7.0, "the_whole_truth", {}),
    Scene("As Above So Below", "as above, so below", 7.0, "as_above_so_below", {}),
    Scene("The Father is the Sun", "as above, so below", 7.0, "the_father_is_the_su", {}),
    Scene("The Mother is the Moon", "as above, so below", 7.0, "the_mother_is_the_mo", {}),
    Scene("The Wind Carried It", "as above, so below", 7.0, "the_wind_carried_it", {}),
    Scene("The Earth is Its Nurse", "as above, so below", 7.0, "the_earth_is_its_nur", {}),
    Scene("Separated from the Gross", "as above, so below", 7.0, "separated_from_the_g", {}),
    Scene("It Ascends and Descends", "as above, so below", 7.0, "it_ascends_and_desce", {}),
    Scene("The Strength of All Strength", "as above, so below", 7.0, "the_strength_of_all_", {}),
    Scene("The World Was Created", "as above, so below", 7.0, "the_world_was_create", {}),
    Scene("Transformed into One", "as above, so below", 7.0, "transformed_into_one", {}),
    Scene("Leaving the Gross Behind", "as above, so below", 7.0, "leaving_the_gross_be", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"emerald_tablet.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Emerald Tablet","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Emerald Tablet",
        "palette_roles":{"gold":"as above","cyan":"as below"},"scenes":recs},
