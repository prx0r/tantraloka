...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_diagram_becomes_alive"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v01(im,u,t,p):
    ...
    ct(d,(cx,cy),"Inert",lf(FSB,24),INK)
    seal(im,"Inert","A diagram is inert until it is given a world — a map without territory is just lines.",GOLD)

def v02(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"World",lf(FSB,24),INK)
    seal(im,"World","The world provides the constraints that make the diagram meaningful — resistance, consequence.",GOLD)

def v03(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Ritual",lf(FSB,24),INK)
    seal(im,"Ritual","A diagram becomes alive when it is given a world — the yantra is inert until the ritual activates it.",GOLD)
...
def v04(im,u,t,p):
    ...
    seal(im,"Closing","A diagram becomes alive when given a world — consciousness is the world that makes the mind real.",GOLD)

SCENES=[
    Scene("Inert","A diagram is inert until it is given a world — a map without territory is just lines.",7.0,"v01",{}),
    Scene("World","The world provides the constraints that make the diagram meaningful — resistance, consequence.",7.0,"v02",{}),
    Scene("Ritual","A diagram becomes alive when it is given a world — the yantra is inert until the ritual activates it.",7.5,"v03",{}),
    Scene("Closing","A diagram becomes alive when given a world — consciousness is the world that makes the mind real.",8.0,"v04",{}),
]
