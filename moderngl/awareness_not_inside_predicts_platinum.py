...
OUTPUT
------
output_awareness_not_inside_predicts/
...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_awareness_not_inside_predicts"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v01(im,u,t,p):
    ...
    ct(d,(cx,cy),"Prediction Machine",lf(FSB,24),INK)
    seal(im,"Prediction Machine","The brain is a prediction engine — it models the world before the world arrives.",GOLD)

def v02(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Model Not Source",lf(FSB,24),INK)
    seal(im,"Model Not Source","Awareness is not inside the model — the model appears within awareness.",GOLD)

def v03(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Light and Castle",lf(FSB,24),INK)
    seal(im,"Light and Castle","The model predicts the castle — the light by which you see is not part of the prediction.",GOLD)
...
def v04(im,u,t,p):
    ...
    seal(im,"Closing","Awareness is not inside the system that predicts the world — the system is inside awareness.",GOLD)

SCENES=[
    Scene("Prediction Machine","The brain is a prediction engine — it models the world before the world arrives.",7.0,"v01",{}),
    Scene("Model Not Source","Awareness is not inside the model — the model appears within awareness.",7.5,"v02",{}),
    Scene("Light and Castle","The model predicts the castle — the light by which you see is not part of the prediction.",7.5,"v03",{}),
    Scene("Closing","Awareness is not inside the system that predicts the world — the system is inside awareness.",8.0,"v04",{}),
]
