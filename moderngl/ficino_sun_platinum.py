#!/usr/bin/env python3
"""PLATINUM PACK -- The Sun That Knows Itself"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_ficino_sun")
...
def v_the_sun_as_face_of_g(im, u, t, p):
    ...
    seal(im,"The Sun as Face of God","solar magic")

def v_the_eye_of_the_world(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Eye of the World","solar magic")

def v_source_of_life(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Source of Life","solar magic")
...
def v_solar_hymn(im, u, t, p):
    ...
    for j in range(int(10*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Solar Hymn","solar magic")

def v_solar_soul(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Solar Soul","solar magic")

def v_the_sun_that_knows(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Sun That Knows","solar magic")

def v_solar_spiritus(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Solar Spiritus","solar magic")

def v_ascent_to_the_sun(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Ascent to the Sun","solar magic")

def v_contemplation(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Contemplation","solar magic")
...
def v_unity(im, u, t, p):
    ...
    for j in range(int(10*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Unity","solar magic")

VISUALS = {
    "the_sun_as_face_of_g": v_the_sun_as_face_of_g,
    "the_eye_of_the_world": v_the_eye_of_the_world,
    "source_of_life": v_source_of_life,
    "solar_hymn": v_solar_hymn,
    "solar_soul": v_solar_soul,
    "the_sun_that_knows": v_the_sun_that_knows,
    "solar_spiritus": v_solar_spiritus,
    "ascent_to_the_sun": v_ascent_to_the_sun,
    "contemplation": v_contemplation,
    "unity": v_unity,
}
...
SCENES = [
    Scene("The Sun as Face of God", "solar magic", 7.0, "the_sun_as_face_of_g", {}),
    Scene("The Eye of the World", "solar magic", 7.0, "the_eye_of_the_world", {}),
    Scene("Source of Life", "solar magic", 7.0, "source_of_life", {}),
    Scene("Solar Hymn", "solar magic", 7.0, "solar_hymn", {}),
    Scene("Solar Soul", "solar magic", 7.0, "solar_soul", {}),
    Scene("The Sun That Knows", "solar magic", 7.0, "the_sun_that_knows", {}),
    Scene("Solar Spiritus", "solar magic", 7.0, "solar_spiritus", {}),
    Scene("Ascent to the Sun", "solar magic", 7.0, "ascent_to_the_sun", {}),
    Scene("Contemplation", "solar magic", 7.0, "contemplation", {}),
    Scene("Unity", "solar magic", 7.0, "unity", {}),
]
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Sun That Knows Itself","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Sun That Knows Itself",
        "palette_roles":{"gold":"the sun","cyan":"eye","violet":"soul"},"scenes":recs},
