#!/usr/bin/env python3
"""THE EMBRYO FLASHES BEFORE IT HAS A BRAIN — Platinum visual essay"""
...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_embryo_flashes"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v_calcium_flash(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-100,cy-90,cx+100,cy+90),outline=(*INK,int(160*pr)),width=3)
    for i in range(12):
        a=i*2*math.pi/12; r=lerp(10,85,pr)
        x=cx+math.cos(a)*r; y=cy+math.sin(a)*r*.7
        if pr>.1: d.ellipse((x-4,y-4,x+4,y+4),fill=(*mix(TEAL,GOLD,i/11),int(160*pr)))
    gc(im,cx,cy,25,GL,int(100*pr),16)
    seal(im,"THE EMBRYO FLASHES BEFORE IT HAS A BRAIN","calcium signalling waves cross the egg — the first body-wide communication")

def v_polarity(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    d.ellipse((cx-90,cy-80,cx+90,cy+80),outline=(*INK,int(160*pr)),width=3)
    for i in range(2):
        x=cx+(i-.5)*60; q=clamp(pr*1.3-i*.1)
        if q<=0: continue
        col=[TEAL,CRIMSON][i]; d.ellipse((x-20,y-20,x+20,y+20),outline=(*col,int(170*q)),width=2)
        ct(d,(x,y),["ANIMAL","VEGETAL"][i],lf(FSNB,int(h*.015)),col)
    seal(im,"ONE AXIS SELECTED FROM COMPETING STREAKS","the first symmetry breaking — a body axis chosen from possibility")

def v_collective(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    for i in range(15):
        x=lerp(200,1080,i/14); y=cy+40*math.sin(i*.9+t)*pr
        d.ellipse((x-5,y-5,x+5,y+5),fill=(*mix(TEAL,GOLD,i/14),int(140*pr)))
    gl(im,pp([(200,cy+40*math.sin(t)),(640,cy-20*pr),(1080,cy+40*math.sin(t+1))],pr),TEAL,3,12,170)
    seal(im,"EMBRYOS LINKED THROUGH ATP AND CALCIUM SIGNALS","the collective resists what one cannot — teratogen resistance through tissue communication")

def v_closing(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.42; pr=ease(u)
    for i,(r,c) in enumerate([(200,GOLD),(150,TEAL),(100,VIOLET),(50,ROSE)]): rr=r*pr; d.ellipse((cx-rr,cy-60-rr*.5,cx+rr,cy-60+rr*.5),outline=(*c,int(120-20*(r//50))),width=2)
    ...
    seal(im,"THE EMBRYO FLASHES BEFORE IT HAS A BRAIN","the body knows before the mind arrives — calcium is the first language")

VISUALS={"calcium_flash":v_calcium_flash,"polarity":v_polarity,"collective":v_collective,"closing":v_closing}
SCENES=[
    Scene("Calcium Flash","The embryo flashes before it has a brain — calcium signalling waves cross the egg.",5.5,"calcium_flash",{}),
    Scene("First Signal","The first body-wide communication — before nerves, before brain.",5.5,"calcium_flash",{}),
    Scene("Axis","One body axis selected from competing streaks — the first symmetry breaking.",5.5,"polarity",{}),
    Scene("BMP","BMP regulation organizes dorsal-ventral polarity — proteins interpret the calcium landscape.",5.5,"polarity",{}),
    Scene("Collective","Embryos linked through ATP and calcium signals — collective teratogen resistance.",5.5,"collective",{}),
    Scene("Communication","The collective resists what one cannot — tissue communication before individuation.",5.5,"collective",{}),
    Scene("Closing","The embryo flashes before it has a brain — the body knows before the mind arrives.",6.0,"closing",{}),
    Scene("Final","Calcium is the first language — before the brain, the body already speaks.",6.5,"closing",{}),
]
...
def export_tl():
    ...
    p=O/"narration_timeline.json"; p.write_text(json.dumps({"title":"the embryo flashes before it has a brain","runtime_seconds":round(cur,3),"scene_count":len(SCENES),"scenes":pl},indent=2,ensure_ascii=False),encoding="utf-8"); return p
def cs(w,h):
