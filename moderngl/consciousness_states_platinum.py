#!/usr/bin/env python3
"""PLATINUM PACK -- The Five States of Consciousness"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_consciousness_states")
...
def v_jagrat_the_waking_st(im, u, t, p):
    ...
    seal(im,"Jagrat The Waking State","waking through turiyatita")

def v_svapna_the_dream_sta(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Svapna The Dream State","waking through turiyatita")

def v_sushupti_deep_sleep(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Sushupti Deep Sleep","waking through turiyatita")

def v_turiya_the_fourth(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Turiya The Fourth","waking through turiyatita")

def v_turiyatita_beyond(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Turiyatita Beyond","waking through turiyatita")

def v_interpenetration_of_(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Interpenetration of States","waking through turiyatita")

def v_the_seven_perceivers(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Seven Perceivers","waking through turiyatita")

def v_the_fifteen_division(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Fifteen Divisions","waking through turiyatita")

def v_breath_as_the_bridge(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Breath as the Bridge","waking through turiyatita")

def v_awakening_within(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Awakening Within","waking through turiyatita")

VISUALS = {
    "jagrat_the_waking_st": v_jagrat_the_waking_st,
    "svapna_the_dream_sta": v_svapna_the_dream_sta,
    "sushupti_deep_sleep": v_sushupti_deep_sleep,
    "turiya_the_fourth": v_turiya_the_fourth,
    "turiyatita_beyond": v_turiyatita_beyond,
    "interpenetration_of_": v_interpenetration_of_,
    "the_seven_perceivers": v_the_seven_perceivers,
    "the_fifteen_division": v_the_fifteen_division,
    "breath_as_the_bridge": v_breath_as_the_bridge,
    "awakening_within": v_awakening_within,
}
...
SCENES = [
    Scene("Jagrat The Waking State", "waking through turiyatita", 7.0, "jagrat_the_waking_st", {}),
    Scene("Svapna The Dream State", "waking through turiyatita", 7.0, "svapna_the_dream_sta", {}),
    Scene("Sushupti Deep Sleep", "waking through turiyatita", 7.0, "sushupti_deep_sleep", {}),
    Scene("Turiya The Fourth", "waking through turiyatita", 7.0, "turiya_the_fourth", {}),
    Scene("Turiyatita Beyond", "waking through turiyatita", 7.0, "turiyatita_beyond", {}),
    Scene("Interpenetration of States", "waking through turiyatita", 7.0, "interpenetration_of_", {}),
    Scene("The Seven Perceivers", "waking through turiyatita", 7.0, "the_seven_perceivers", {}),
    Scene("The Fifteen Divisions", "waking through turiyatita", 7.0, "the_fifteen_division", {}),
    Scene("Breath as the Bridge", "waking through turiyatita", 7.0, "breath_as_the_bridge", {}),
    Scene("Awakening Within", "waking through turiyatita", 7.0, "awakening_within", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"consciousness_states.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Five States of Consciousness","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Five States of Consciousness",
        "palette_roles":{"ink":"waking","cyan":"dreaming","violet":"sleep","gold":"turiya"},"scenes":recs},
