...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_cells_solving_problem"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v01(im,u,t,p):
    ...
    ct(d,(cx,cy),"Cell Problem",lf(FSB,24),INK)
    seal(im,"Cell Problem","Your cells may be solving a problem called you — each one contributes to a pattern larger than itself.",GOLD)

def v02(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Bioelectric",lf(FSB,24),INK)
    seal(im,"Bioelectric","Bioelectric networks coordinate across thousands of cells — the problem is solved at tissue level.",GOLD)

def v03(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Persistence",lf(FSB,24),INK)
    seal(im,"Persistence","The problem is persistence — maintaining a body through time despite constant cellular turnover.",GOLD)
...
def v04(im,u,t,p):
    ...
    seal(im,"Closing","Your cells may be solving a problem called you — the solution is the life you are living.",GOLD)

SCENES=[
    Scene("Cell Problem","Your cells may be solving a problem called you — each one contributes to a pattern larger than itself.",7.0,"v01",{}),
    Scene("Bioelectric","Bioelectric networks coordinate across thousands of cells — the problem is solved at tissue level.",7.5,"v02",{}),
    Scene("Persistence","The problem is persistence — maintaining a body through time despite constant cellular turnover.",7.5,"v03",{}),
    Scene("Closing","Your cells may be solving a problem called you — the solution is the life you are living.",8.0,"v04",{}),
]
