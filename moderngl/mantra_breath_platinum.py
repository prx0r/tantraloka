#!/usr/bin/env python3
"""PLATINUM PACK -- The Pulse of Consciousness"""
...
OUTPUT = Path("/mnt/HC_Volume_106427611/goldrender/output_mantra_breath")
...
def v_cakrodaya_the_wheel(im, u, t, p):
    ...
    seal(im,"Cakrodaya The Wheel","spanda breath mantra")

def v_spanda_the_pulse(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Spanda The Pulse","spanda breath mantra")

def v_the_waterwheel_simil(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Waterwheel Simile","spanda breath mantra")

def v_breath_as_mantra(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Breath as Mantra","spanda breath mantra")

def v_breath_mantra_synchr(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Breath-Mantra Synchrony","spanda breath mantra")

def v_kalagrasa_consuming_(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Kalagrasa Consuming Time","spanda breath mantra")

def v_spanda_as_perception(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"Spanda as Perception Pulse","spanda breath mantra")

def v_the_power_of_the_voi(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Power of the Void","spanda breath mantra")

def v_the_channels_of_brea(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Channels of Breath","spanda breath mantra")

def v_the_pulse_of_conscio(im, u, t, p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*0.50,h*0.42; r=ease(u)
    for j in range(int(10*r)):
        a=j*math.tau/10+t*0.2; rr=25+20*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*0.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,15,GOLD,int(150*r),12)
    seal(im,"The Pulse of Consciousness","spanda breath mantra")

VISUALS = {
    "cakrodaya_the_wheel": v_cakrodaya_the_wheel,
    "spanda_the_pulse": v_spanda_the_pulse,
    "the_waterwheel_simil": v_the_waterwheel_simil,
    "breath_as_mantra": v_breath_as_mantra,
    "breath_mantra_synchr": v_breath_mantra_synchr,
    "kalagrasa_consuming_": v_kalagrasa_consuming_,
    "spanda_as_perception": v_spanda_as_perception,
    "the_power_of_the_voi": v_the_power_of_the_voi,
    "the_channels_of_brea": v_the_channels_of_brea,
    "the_pulse_of_conscio": v_the_pulse_of_conscio,
}
...
SCENES = [
    Scene("Cakrodaya The Wheel", "spanda breath mantra", 7.0, "cakrodaya_the_wheel", {}),
    Scene("Spanda The Pulse", "spanda breath mantra", 7.0, "spanda_the_pulse", {}),
    Scene("The Waterwheel Simile", "spanda breath mantra", 7.0, "the_waterwheel_simil", {}),
    Scene("Breath as Mantra", "spanda breath mantra", 7.0, "breath_as_mantra", {}),
    Scene("Breath-Mantra Synchrony", "spanda breath mantra", 7.0, "breath_mantra_synchr", {}),
    Scene("Kalagrasa Consuming Time", "spanda breath mantra", 7.0, "kalagrasa_consuming_", {}),
    Scene("Spanda as Perception Pulse", "spanda breath mantra", 7.0, "spanda_as_perception", {}),
    Scene("The Power of the Void", "spanda breath mantra", 7.0, "the_power_of_the_voi", {}),
    Scene("The Channels of Breath", "spanda breath mantra", 7.0, "the_channels_of_brea", {}),
    Scene("The Pulse of Consciousness", "spanda breath mantra", 7.0, "the_pulse_of_conscio", {}),
]
...
def concat(paths):
    ...
    final=OUTPUT/"mantra_breath.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"The Pulse of Consciousness","scene_count":len(SCENES),
        "runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],
        "continuity_object":"SUBThe Pulse of Consciousness",
        "palette_roles":{"gold":"spanda","cyan":"breath","violet":"mantra"},"scenes":recs},
