...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_self_world"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
SCENES=[
    Scene("Reeds","Place two bundles of reeds together — lean each against the other.",6.0,"reeds",{}),
    Scene("Neither Stands","Neither stands alone — remove one, the other falls.",5.0,"reeds",{}),
    Scene("Dependent Arising","Ñāṇananda uses this image for consciousness and name-and-form.",5.5,"reeds",{}),
    Scene("Reciprocal","Self and world hold each other upright — the relation produces the field.",5.5,"braid",{}),
    Scene("Consciousness","Consciousness is not an abstract light floating outside conditions — it arises in relation to structured experience.",5.5,"braid",{}),
    Scene("Not Construction","The mind does not invent the world from nothing — a body encounters structures not chosen by thought.",5.5,"not_construction",{}),
    Scene("Affordance","A stair affords climbing — a face affords approach or avoidance — a noise becomes language to one listener.",5.5,"affordance",{}),
    Scene("Enactment","Perception is not passive reception — it is skillful engagement.",5.5,"enaction_cycle",{}),
    Scene("Action","The organism discovers a world through possible action — vision changes with movement.",5.5,"enaction_cycle",{}),
    Scene("Counterfactual","The hidden side of a cup is present as a counterfactual: if I rotate it, another surface will appear.",5.5,"hidden_side",{}),
    Scene("Enactive AI","A 2026 paper on enactive AI emphasizes autonomy, embodiment, and intrinsic normativity.",5.5,"ai_boundary",{}),
    Scene("Reward","A reward function from outside is not identical to a need generated by a system whose existence is at stake.",5.5,"ai_boundary",{}),
    Scene("Self-Model World-Model","A self-model depends upon a world-model — each pole confirms the other.",5.5,"mirrors",{}),
    Scene("Loop","The loop becomes stable enough to feel discovered rather than constructed.",5.5,"mirrors",{}),
    Scene("Grasping","Grasping hardens the loop — a painful self-model predicts rejection, attention finds signs, action becomes defensive.",6.0,"grasping",{}),
    Scene("Self-Evidencing","The prediction gains evidence because it created the conditions from which evidence was sampled.",5.5,"self_evidencing",{}),
    Scene("Vortex","The self is repeatedly rebuilt through mine, for me, against me, what I must become.",5.5,"grasping",{}),
    Scene("Loosening","Practice works at the loop — change the environment, the self receives new evidence.",5.5,"loosening",{}),
    Scene("Meditation","Meditation adds a radical experiment — let the world appear, let the self-model appear.",5.5,"ground",{}),
    Scene("Dependence Becomes Visible","The poles depend upon one another — neither stands alone.",5.0,"ground",{}),
    Scene("Kashmir Śaiva Ground","For Abhinavagupta, the collapse of false limitation reveals consciousness's universal reflexivity.",5.5,"ground",{}),
    Scene("Closing","Self and world hold each other upright — remove one, the other falls.",6.0,"closing",{}),
    Scene("Final","The error is not that the structures function — it is believing either possesses independent solidity.",6.5,"closing",{}),
]
...
def export_tl():
    ...
    p=O/"narration_timeline.json"; p.write_text(json.dumps({"title":"self and world hold each other upright","runtime_seconds":round(cur,3),"scene_count":len(SCENES),"scenes":pl},indent=2,ensure_ascii=False),encoding="utf-8"); return p
def cs(w,h):
