...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_leaf_uses_noise"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v01(im,u,t,p):
    ...
    ct(d,(cx,cy),"Noise",lf(FSB,24),INK)
    seal(im,"Noise","A leaf uses noise to keep light from getting lost — fluctuations are not obstacles.",GOLD)

def v02(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Exciton",lf(FSB,24),INK)
    seal(im,"Exciton","Excitons travel through the pigment network — noise helps them escape traps.",GOLD)

def v03(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Coherence",lf(FSB,24),INK)
    seal(im,"Coherence","Brief quantum coherence guides energy toward the reaction centre — noise sustains the path.",GOLD)
...
def v04(im,u,t,p):
    ...
    seal(im,"Closing","A leaf uses noise to keep light from getting lost — life exploits what physics does not forbid.",GOLD)

SCENES=[
    Scene("Noise","A leaf uses noise to keep light from getting lost — fluctuations are not obstacles.",7.0,"v01",{}),
    Scene("Exciton","Excitons travel through the pigment network — noise helps them escape traps.",7.0,"v02",{}),
    Scene("Coherence","Brief quantum coherence guides energy toward the reaction centre — noise sustains the path.",7.5,"v03",{}),
    Scene("Closing","A leaf uses noise to keep light from getting lost — life exploits what physics does not forbid.",8.0,"v04",{}),
]
