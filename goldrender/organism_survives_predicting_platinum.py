...
OUTPUT
------
output_organism_survives_predicting/
...
ROOT=Path(__file__).resolve().parent; O=ROOT/"output_organism_survives_predicting"; FRAMES=O/"frames"; SCENES_DIR=O/"scenes"
W,H,FPS=1280,720,10
...
def v01(im,u,t,p):
    ...
    ct(d,(cx,cy),"Self Prediction",lf(FSB,24),INK)
    seal(im,"Self Prediction","An organism survives by predicting itself — every action is a hypothesis about viability.",GOLD)

def v02(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Loop",lf(FSB,24),INK)
    seal(im,"Loop","Prediction forms a loop: act, sense, update, act again. The self is enacted through time.",GOLD)

def v03(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.5,h*.40; q=ease(u)
    d.ellipse((cx-70,cy-50,cx+70,cy+50),outline=(*GOLD,int(160*q)),width=3)
    gc(im,cx,cy,20,GL,int(100*q),16)
    ct(d,(cx,cy),"Error Signal",lf(FSB,24),INK)
    seal(im,"Error Signal","Prediction error is not failure — it is the signal that drives adaptation.",GOLD)
...
def v04(im,u,t,p):
    ...
    seal(im,"Closing","An organism survives by predicting itself — the self is the question the body answers by continuing.",GOLD)

SCENES=[
    Scene("Self Prediction","An organism survives by predicting itself — every action is a hypothesis about viability.",7.0,"v01",{}),
    Scene("Loop","Prediction forms a loop: act, sense, update, act again. The self is enacted through time.",7.0,"v02",{}),
    Scene("Error Signal","Prediction error is not failure — it is the signal that drives adaptation.",7.0,"v03",{}),
    Scene("Closing","An organism survives by predicting itself — the self is the question the body answers by continuing.",8.0,"v04",{}),
]
